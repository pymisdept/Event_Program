	...
	<div class="bg-contact3" style="background-image: url('../images/luckydraw(Covid19)05bg_ch1.jpg');">
			...
			<div class="wrap-contact4">
				<form class="contact3-form validate-form" method="post" id="myForm" name="myForm" runat="server" >
				   ...
				   <div class="form-group row">
					...
					<div class="col-sm-8">
						<asp:TextBox type="text" runat="server" class="form-control" id="post" name="post" placeholder="" maxlength="100" required/>
					</div>
				  </div>
					
					<div class="form-group row">
					<label for="post" class="col-sm-4 col-form-label"><span data-localize="name">員工編號 ﹕</span><span style="color:red">*</span></label>
					<div class="col-sm-8">
				   ...
				   <div class="form-group row">
					...
					<div class="col-sm-8">
						<asp:TextBox type="text" runat="server" class="form-control" id="hkid" name="hkid" placeholder="" maxlength="100" required/>
					</div>
				  </div>
    	...
    	<script>
    	    function success(){
                ...
                function wrong(){
                	if (lang=='eng'){
                		swal('Invalid Staff No. (5 digits) or Hong Kong ID Card No. (First 4 letters & digits) !','', 'error');
                	}
                	else{
                		swal('員工編號 (5位數字) 或 香港身份證號碼 (英文字母及首3個數字) 錯誤 !','', 'error');
                	}    	
        }
        if(status=='hr'){
            	hr();
